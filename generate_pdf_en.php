...
// 获取空闲数量
$totalFree = $db->getTotalFree($lang);
if ($totalFree == 0) {
...
// 获取是否在处理中
$isProcessing = $db->getProcessing($lang);
if ($isProcessing) {
	die('Processing, skip...');
}

// 若非处理中，则处理下一条PDF生成
$row = $db->get_one('`status` = 0 AND `lang` = 2', 'id, product_id', 'id DESC');
...
$product_props = $db_linkage->product_props_en();
...
$pdf_path = __DIR__ . '/caches/pdf/en/g/' . $code_j . '.pdf';
$pdf_path_noj = __DIR__ . '/caches/pdf/en/ng/' . $code_noj . '.pdf';
$pdf_template = __DIR__.'/phpcms/modules'.DIRECTORY_SEPARATOR.'product'.DIRECTORY_SEPARATOR.'pdf.template.en';
...
$pdf_j_content = preg_replace('/{title}/', $product['title'] . '<br />' . $code_j, $pdf_content_with_j);

$pdf_content_without_j = preg_replace('/{military_standard}/', '-', $pdf_content_without_j);
$pdf_noj_content = preg_replace('/{title}/', $product['title'] . '<br />' . $code_noj, $pdf_content_without_j);
