...
class member extends admin {
	...
	public function orders() {
        ...
        $address = file_get_contents(CACHE_PATH . 'address.data');
        ...
        if (!empty($keyword)) {
            $where .= '(order_no LIKE \'%' . $keyword . '%\' OR `name` LIKE \'%' . $keyword . '%\' OR phone LIKE \'%' . $keyword . '%\' OR addressee LIKE \'%' . $keyword . '%\')';
        }
        ...
        if (!empty($start_time)) {
            ...
            $where .= 'order_time >= ' . $where_start_time;
        }

        if (!empty($end_time)) {
            $where_end_time = strtotime('+1 day', strtotime($end_time));
            $where .= 'order_time < ' . $where_end_time;
        }

        $list_arr = $order_db->listinfo($where, 'id DESC', $page, 15);
        $pages = $this->db->pages;
            ...
            foreach ($list_arr as $item) {
                ...
                if (!empty($province_id)) {
                    foreach ($address as $addr) {
                        if ($addr['id'] == $province_id) {
                            $province = $addr;
                            $province_name = $addr['name'];
                            break;
                        }
                    }
                }
                ...
                if (!empty($province['child'])) {
                    foreach ($province['child'] as $value_city) {
                        if ($value_city['id'] == $city_id) {
                            $city = $value_city;
                            $city_name = $value_city['name'];
                            break;
                        }
                    }
                }
                ...
                if (!empty($city['child'])) {
                    foreach ($city['child'] as $value_county) {
                        if ($value_county['id'] == $county_id) {
                            $county_name = $value_county['name'];
                            break;
                        }
                    }
                }
                ...
                $order_list[] = [
                    ...
                    'name' => $item['name'],
                    'phone' => $item['phone'],
                    'addressee' => $province_name . $city_name . $county_name . $item['addressee'],
                    'order_time' => date('Y-m-d H:i:s', $item['order_time']),
                    'order_status' => $status_map[$item['order_status']],
                ];
            }
        }
        include $this->admin_tpl('member_order');
    }

    public function order_dispatch() {
        $order_db = pc_base::load_model('order_model');
        $id = (int)$_POST['id'];
        $order_db->update(array('order_status' => 2), array('id' => $id));
    }

    public function order_remove() {
        $order_db = pc_base::load_model('order_model');
        $id = (int)$_POST['id'];
        $order_db->update(array('order_status' => -1), array('id' => $id));
    }
    ...
    public function order_detail() {
        ...
        $info['addressee'] = $province_name . $city_name . $county_name . $info['addressee'];
        $info['order_status'] = $status_map[$info['order_status']];
        $info['order_time'] = date('Y-m-d H:i:s', $info['order_time']);

        $order_goods_db = pc_base::load_model('orderdetail_model');
        $sql  = 'SELECT g.title,d.total,d.price ';
        $sql .= 'FROM lldz_order_detail d ';
        $sql .= 'LEFT JOIN lldz_order o ON o.id = d.order_id ';
        $sql .= 'LEFT JOIN lldz_lldz_goods g ON g.id = d.goods_id';
        $order_goods_db->query($sql);
        $arr_order_detail = $order_goods_db->fetch_array();

        include $this->admin_tpl('member_order_detail');
    }
