...
class index extends foreground {
    ...
    function __construct() {
        ...
        $this->_curl = pc_base::load_sys_class('curl');
        $this->sms_sign = pc_base::load_sys_class('ali_sms_sign');

        $this->_order_db = pc_base::load_model("order_model");
    }
    ...
    public function init() {
        ...
        // 获取用户订单
        $sql  = 'SELECT o.id,o.order_no,g.title,o.pay_type,o.order_sum,o.order_status ';
        $sql .= 'FROM lldz_order o ';
        $sql .= 'LEFT JOIN lldz_order_detail d ON o.order_no = d.order_no ';
        $sql .= 'LEFT JOIN lldz_lldz_goods g ON g.id = d.goods_id ';
        $sql .= 'WHERE o.member_id = ' . $memberinfo['userid'];
        $this->_order_db->query($sql);
        $orders = $this->_order_db->fetch_array();
    ...
    public function login() {
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            $username = isset($_POST['phone']) && is_phone($_POST['phone']) ? trim($_POST['phone']) : showmessage(L('请输入手机号'), HTTP_REFERER);
            ...
            if(pc_base::load_config('system', 'phpsso')) {
                //phsoo登录方式
                is_code($_POST['code']) && is_badword($_POST['code'])==false ? trim($_POST['code']) : showmessage(L('code_error'), HTTP_REFERER);
                ...
                $result = $sms_content_db->get_one("telephone = '$username'",'id,telephone,code','created_at desc' );
                //验证用户验证码
                /*$password = md5(md5(trim($password)).$r['encrypt']);*/
                if($result['code'] != $code || $result['telephone']!=$username) {
                    $ip = ip();
                    ...
                    showmessage(L('验证码错误', array('times'=>$times)), 'index.php?m=member&c=index&a=login', 3000);
                }
            ...
            $_cookietime = $cookietime ? intval($cookietime) : ($get_cookietime ? $get_cookietime : 0);
            $cookietime = $_cookietime ? SYS_TIME + $_c+ookietime : 0;
...
//    var_dump($telephone);
        ...
        $params["TemplateCode"] = "SMS_119086775";
...
//        $helper = new SignatureHelper();
    ...
    public function order_detail() {
        ...
        include template('member', 'order_detail');
    }
}
?>