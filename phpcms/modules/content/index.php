...
class index {
	...
	public function show() {
        ...
        $positions = array();
        if ($catid == 10) {
		    $sql  = 'SELECT g.url,g.thumb,gd.price,g.id ';
		    $sql .= 'FROM lldz_lldz_goods g ';
		    $sql .= 'LEFT JOIN lldz_lldz_goods_data gd ON g.id = gd.id ';
		    $sql .= 'WHERE g.posids = 1 AND g.catid = ' . $catid;
		    $this->db->query($sql);
		    $recommand = $this->db->fetch_array();

		    if (!empty($recommand)) {
		        foreach ($recommand as $item) {
                    // 获取销量
                    $sql  = 'SELECT COUNT(od.id) AS sale ';
                    $sql .= 'FROM lldz_order_detail od ';
                    $sql .= 'LEFT JOIN lldz_order o ON o.id = od.order_id ';
                    $sql .= 'WHERE od.goods_id = ' . $item['id'] . ' AND o.order_status = 1';
                    $this->db->query($sql);
                    $sale = $this->db->fetch_array();
